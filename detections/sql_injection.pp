locals {
  sql_injection_common_tags = merge(local.aws_alb_access_log_detections_common_tags, {
    category = "SQL Injection"
  })
}

benchmark "sql_injection_detections" {
  title       = "SQL Injection (SQLi) Detections"
  description = "This benchmark contains SQLi focused detections when scanning access logs."
  type        = "detection"
  children = [
    detection.sql_injection_blind_based,
    detection.sql_injection_common_patterns,
    detection.sql_injection_error_based,
    detection.sql_injection_time_based,
    detection.sql_injection_union_based,
    detection.sql_injection_user_agent_based,
  ]

  tags = merge(local.sql_injection_common_tags, {
    type = "Benchmark"
  })
}

detection "sql_injection_common_patterns" {
  title           = "SQL Injection Common Patterns"
  description     = "Detect basic SQL injection attempts targeting common SQL keywords and syntax patterns that might indicate an attempt to manipulate database queries."
  documentation   = file("./detections/docs/sql_injection_common_patterns.md")
  severity        = "critical"
  display_columns = local.detection_display_columns

  query = query.sql_injection_common_patterns

  tags = merge(local.sql_injection_common_tags, {
    mitre_attack_ids = "TA0001:T1190",
    owasp_top_10     = "A03:2021-Injection"
  })
}

query "sql_injection_common_patterns" {
  sql = <<-EOQ
    select
      ${local.detection_sql_columns}
    from
      aws_alb_access_log
    where
      request_url is not null
      and (
        -- Basic SQL commands
        request_url ilike '%select%from%'
        or request_url ilike '%insert%into%'
        or request_url ilike '%delete%from%'
        or request_url ilike '%update%set%'
        or request_url ilike '%drop%table%'
        or request_url ilike '%truncate%table%'
        or request_url ilike '%create%table%'
        or request_url ilike '%alter%table%'
        or request_url ilike '%exec%xp_%'
        or request_url ilike '%information_schema%'
        -- Common SQL injection patterns
        or request_url ilike '%or%1=1%'
        or request_url ilike '%or%true%'
        or request_url ilike '%/*_%*/%'
        or request_url ilike '%--+%'
        or request_url ilike '%-- %'
        or request_url ilike '%;--%'
        -- URL encoded variants
        or request_url ilike '%\x27%'
        or request_url ilike '%\x22%'
        or request_url ilike '%\x3D\x3D%'
      )
    order by
      tp_timestamp desc;
  EOQ
}

detection "sql_injection_union_based" {
  title           = "SQL Injection Union Based"
  description     = "Detect UNION-based SQL injection attacks that attempt to join results from another query to the original query's results."
  documentation   = file("./detections/docs/sql_injection_union_based.md")
  severity        = "critical"
  display_columns = local.detection_display_columns

  query = query.sql_injection_union_based

  tags = merge(local.sql_injection_common_tags, {
    mitre_attack_ids = "TA0001:T1190",
    owasp_top_10     = "A03:2021-Injection"
  })
}

query "sql_injection_union_based" {
  sql = <<-EOQ
    select
      ${local.detection_sql_columns}
    from
      aws_alb_access_log
    where
      request_url is not null
      and (
        -- UNION-based patterns
        request_url ilike '%union%select%'
        -- Evasion techniques specific to UNION
        or request_url ilike '%uni%on%sel%ect%'
        or request_url ilike '%uni*/*/on/**/sel/**/ect%'
        or request_url ilike '%un?on+sel?ct%'
      )
    order by
      tp_timestamp desc;
  EOQ
}

detection "sql_injection_blind_based" {
  title           = "SQL Injection Blind Based"
  description     = "Detect blind SQL injection attacks that attempt to extract information from the database using boolean conditions or time delays."
  documentation   = file("./detections/docs/sql_injection_blind_based.md")
  severity        = "critical"
  display_columns = local.detection_display_columns

  query = query.sql_injection_blind_based

  tags = merge(local.sql_injection_common_tags, {
    mitre_attack_ids = "TA0001:T1190",
    owasp_top_10     = "A03:2021-Injection"
  })
}

query "sql_injection_blind_based" {
  sql = <<-EOQ
    select
      ${local.detection_sql_columns}
    from
      aws_alb_access_log
    where
      request_url is not null
      and (
        -- Blind condition checks
        request_url ilike '%and%1=1%'
        or request_url ilike '%and%1=2%'
        or request_url ilike '%case%when%'
        or request_url ilike '%substr%(%'
        or request_url ilike '%substring%(%'
        or request_url ilike '%ascii%(%'
        or request_url ilike '%length%(%'
        or request_url ilike '%benchmark%(%'
        -- Blind patterns with comparison operators
        or request_url ilike '%and+1>0%'
        or request_url ilike '%and+1<2%'
        or request_url ilike '%and+ascii(substring%'
        or request_url ilike '%and+length(%)%'
        -- URL encoded variants common in blind injections
        or request_url ilike '%and%28select%'
        or request_url ilike '%and%28case%'
      )
      and request_url not ilike '%rand%'
    order by
      tp_timestamp desc;
  EOQ
}

detection "sql_injection_error_based" {
  title           = "SQL Injection Error Based"
  description     = "Detect error-based SQL injection attacks that attempt to extract information from database error messages."
  documentation   = file("./detections/docs/sql_injection_error_based.md")
  severity        = "critical"
  display_columns = local.detection_display_columns

  query = query.sql_injection_error_based

  tags = merge(local.sql_injection_common_tags, {
    mitre_attack_ids = "TA0001:T1190",
    owasp_top_10     = "A03:2021-Injection"
  })
}

query "sql_injection_error_based" {
  sql = <<-EOQ
    select
      ${local.detection_sql_columns}
    from
      aws_alb_access_log
    where
      request_url is not null
      and (
        -- Error-based extraction patterns
        request_url ilike '%convert%(%'
        or request_url ilike '%cast%(%'
        or request_url ilike '%extractvalue%(%'
        or request_url ilike '%updatexml%(%'
        or request_url ilike '%floor%(%'
        or request_url ilike '%exp%(%'
        or request_url ilike '%concat%(%'
        or request_url ilike '%concat_ws%(%'
        or request_url ilike '%group_concat%(%'
        -- Known error-based functions with database fingerprinting
        or request_url ilike '%db_name%(%'
        or request_url ilike '%@@version%'
        or request_url ilike '%version%(%'
        or request_url ilike '%pg_sleep%(%'
        or request_url ilike '%sys.%'
        -- Common error triggers
        or request_url ilike '%having%1=1%'
        or request_url ilike '%order%by%'
        or request_url ilike '%group%by%'
      )
    order by
      tp_timestamp desc;
  EOQ
}

detection "sql_injection_time_based" {
  title           = "SQL Injection Time Based"
  description     = "Detect time-based SQL injection attacks that attempt to extract information by causing delays in database response times."
  documentation   = file("./detections/docs/sql_injection_time_based.md")
  severity        = "critical"
  display_columns = local.detection_display_columns

  query = query.sql_injection_time_based

  tags = merge(local.sql_injection_common_tags, {
    mitre_attack_ids = "TA0001:T1190",
    owasp_top_10     = "A03:2021-Injection"
  })
}

query "sql_injection_time_based" {
  sql = <<-EOQ
    select
      ${local.detection_sql_columns}
    from
      aws_alb_access_log
    where
      request_url is not null
      and (
        -- Time-based functions for various database types
        request_url ilike '%sleep%(%'
        or request_url ilike '%benchmark%(%'
        or request_url ilike '%pg_sleep%(%'
        or request_url ilike '%dbms_pipe.receive_message%(%'
        or request_url ilike '%waitfor%delay%'
        or request_url ilike '%GENERATE_SERIES%'
      )
    order by
      tp_timestamp desc;
  EOQ
}

detection "sql_injection_user_agent_based" {
  title           = "SQL Injection User Agent Based"
  description     = "Detect SQL injection attacks that use the User-Agent header rather than URL parameters to bypass WAF protections or input filtering."
  documentation   = file("./detections/docs/sql_injection_user_agent_based.md")
  severity        = "critical"
  display_columns = local.detection_display_columns

  query = query.sql_injection_user_agent_based

  tags = merge(local.sql_injection_common_tags, {
    mitre_attack_ids = "TA0001:T1190",
    owasp_top_10     = "A03:2021-Injection"
  })
}

query "sql_injection_user_agent_based" {
  sql = <<-EOQ
    select
      ${local.detection_sql_columns}
    from
      aws_alb_access_log
    where
      user_agent is not null
      and (
        -- Basic SQL injection patterns in User-Agent
        user_agent ilike '%select%from%'
        or user_agent ilike '%union%select%'
        or user_agent ilike '%insert%into%'
        or user_agent ilike '%update%set%'
        or user_agent ilike '%delete%from%'
        or user_agent ilike '%drop%table%'
        -- Common SQL comment markers and logic patterns
        or user_agent ilike '%/*_%*/%'
        or user_agent ilike '%--+%'
        or user_agent ilike '%-- %'
        or user_agent ilike '%;--%'
        or user_agent ilike '%or%1=1%'
        or user_agent ilike '%or%true%'
        -- Database-specific User-Agent attacks
        or user_agent ilike '%@@version%'
        or user_agent ilike '%information_schema%'
        or user_agent ilike '%sql_injectionte_master%'
        or user_agent ilike '%pg_tables%'
        or user_agent ilike '%sys.%'
        -- Time-based techniques
        or user_agent ilike '%sleep(%'
        or user_agent ilike '%benchmark(%'
        or user_agent ilike '%pg_sleep(%'
        or user_agent ilike '%waitfor%delay%'
      )
    order by
      tp_timestamp desc;
  EOQ
} 